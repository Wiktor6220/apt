...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($name)) {
        ...
        try {
            ...
            if ($stmt->execute()) {
                $success = "Apteczka została utworzona pomyślnie.";
            } else {
                $error = "Wystąpił błąd podczas tworzenia apteczki.";
            }
            
            $stmt->close();
            $conn->close();
        } catch (Exception $e) {
            $error = "Wystąpił błąd podczas tworzenia apteczki. Spróbuj ponownie później.";
        }
    }
}
?>
...
<head>
    ...
    <title>Dodaj nową apteczkę - System Zarządzania Apteczkami</title>
            ...
            <div class="col-md-6">
                <div class="card">
                    <div class="card-header">
                        <h3 class="text-center">Dodaj nową apteczkę</h3>
                    </div>
                    <div class="card-body">
                        <?php if ($error): ?>
                        ...
                        <?php else: ?>
                            <form method="POST" action="">
                                ...
                                <div class="d-grid gap-2">
                                    <button type="submit" class="btn btn-primary">Utwórz apteczkę</button>
                                </div>
                            </form>
                        <?php endif; ?>
                    </div>
